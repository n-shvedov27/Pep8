...
class CheckerBlankLines:
    ...
    def _check_two_blank_lines(self, line, vars):
        """
            Прверяет, что перед описанием классса или
            функции(вне класса) есть 2 пустые строки
        """
        ...
        if copy_line.startswith('class') or copy_line.startswith('def'):
            ...
            if n < required_blank_line:
                if not vars.previous_line_contains_class and not vars.begining:
                        error_str = (
                            " E302 expected 2 blank lines, found " + str(n))
