...
class CheckerSpaces:
    ...
    def _check_space_in_end(self, line, vars):
        ...
        if len(line) != 0:
            if line[:-1].endswith(" ") or line.endswith(" \n") or\
                    line.endswith("\t"):
                return [(vars.line_counter, len(line)), " W291 trailing whitespace"]
    ...
    def _check_two_spaces_before_comment(self, line, vars):
        ...
        if index != -1:
            print(line[index + 1])
            if line[index + 1] != " ":
                return [(vars.line_counter, index), " E261 block comment should start with '#'"]
    ...
    def _check_spases_around_keyword_or_equals_parameter(self, line, vars):
        ...
        for char in ['=', '>', '<', '!']:
            ...
            if (line[index - 1] != " " or line[index + 1] != " ") and \
                    ...
                    line[index - 1] != "!" and line[index - 1] != "<" and line[index - 1] != ">":
                return [(vars.line_counter, index), " E251 unexpected spaces around keyword / parameter equals"]
            if line[index-2] == ' ' and not (line[index-1] == '=' or line[index-1] == '<' or line[index-1] == '>'):
                return [(vars.line_counter, index), " E25 multiple space before operators"]
            if line[index+2] == ' ' and line[index+1] != '=':
                return [(vars.line_counter, index), " E251 multiple space after operators"]
    ...
    def _check_mult_four_ident(self, line, vars):
        ...
        while copy_line[0] == ' ':
            ...
            if copy_line == "":
                break
        if index % 4 != 0:
            return [(vars.line_counter, 1), " E111 indentation is not a multiple of four"]
    ...
    def _check_exp_unexp_ident(self, line, vars):
        ...
        if count_indent >= 0:
            ...
            if not line.startswith(prefix) and line != '\n':
                res = [(vars.line_counter, 1), " E112 expected an indented block"]
        else:
            if line.startswith(' '):
    ...
    def _check_space_around_bracket_and_special_chars(self, line, vars):
        ...
        for index in range(len(line)):
            if line[index] in self.open_bracket and line[index+1] == ' ':
                err_str = " E201 whitespace after " + line[index]
                errors.append([(vars.line_counter, index), err_str])
            if line[index] == ' ' and line[index+1] in self.close_bracket_and_special_chars:
